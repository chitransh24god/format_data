...
def render():
    st.markdown('<div class="hero-title">Turn Any Data Into<br><span>Excel. Instantly.</span></div>', unsafe_allow_html=True)
    st.markdown(
    ...
    for i, (icon, title, desc) in enumerate(FEATURES):
        with cols[i % 3]:
            st.markdown(
                f'<div class="df-card"><div style="font-size:1.5rem;margin-bottom:0.5rem">{icon}</div>'
                f'<h4 style="color:#2563eb;margin:0 0 0.4rem;font-size:0.95rem">{title}</h4>'
                f'<p style="color:#475569;font-size:0.85rem;line-height:1.65;margin:0">{desc}</p></div>',
                unsafe_allow_html=True,
            )
